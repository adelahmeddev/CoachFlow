import { Clock3, Users, Flame, ArrowLeft, Zap, Trophy, Target, Dumbbell, Sparkles } from "lucide-react"
    ...
    <div className="space-y-6">
      ...
      <FadeIn>
        <div className="relative overflow-hidden rounded-[24px] border bg-card shadow-soft">
          ...
          <div className="absolute -right-12 -top-12 size-40 rounded-full bg-gradient-to-br from-brand-500/20 to-energy-500/20 blur-3xl animate-ambient" aria-hidden="true" />
          <div className="absolute -left-12 -bottom-12 size-32 rounded-full bg-gradient-to-br from-muscle-500/10 to-brand-500/10 blur-2xl animate-ambient" aria-hidden="true" />
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-3 min-w-0 flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {/* LIVE GYM BEACON */}
                  <div className="inline-flex items-center gap-1.5 rounded-full border border-performance-500/20 bg-performance-500/10 px-2.5 py-0.5 text-xs font-semibold text-performance-700 dark:text-performance-300">
                    <span className="relative flex size-2">
                      <span className="absolute inline-flex size-full animate-beacon rounded-full bg-performance-500 opacity-75" />
                      <span className="relative inline-flex size-2 rounded-full bg-performance-500" />
                    </span>
                    <span>{isAr ? "لايف النهاردة" : "Gym Live Today"}</span>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row lg:flex-col xl:flex-row shrink-0">
                ...
                <Button asChild className="gap-2 rounded-xl bg-gradient-to-r from-brand-600 to-brand-500 shadow-soft hover:brightness-110">
                  <Link href="/clients">
                    ...
                    <ArrowLeft className="size-4 rtl:-scale-x-100 hidden sm:inline" aria-hidden="true" />
                  </Link>
                </Button>
              </div>
            </div>
      ...
      {/* STATS — Fitness variants with StaggerList */}
          ...
          <Card className="overflow-hidden border bg-card shadow-soft">
            <CardHeader className="py-4 border-b bg-gradient-to-r from-brand-500/[0.04] to-transparent">
              <div className="flex min-w-0 items-center gap-2">
                <span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-brand-500 to-energy-500 text-white shadow-soft">
                  <Dumbbell className="size-4" />
                </span>
                <CardTitle className="min-w-0 flex-1 break-words text-sm font-bold tracking-tight">{isAr ? "يومك في الجيم" : "Today in the gym"}</CardTitle>
              </div>
            </CardHeader>
      ...
      {/* ACTION FEED — computed needs-action + today's check-ins */}
      <FadeIn delay={0.25}>
        <div className="grid gap-4 lg:grid-cols-3">
      ...
      {recentClients.length === 0 ? (
        ...
        <FadeIn delay={0.35}>
          <Card className="overflow-hidden border bg-card shadow-soft">
            <CardHeader className="flex flex-row items-center justify-between gap-2 border-b bg-muted/20 py-4">
              <div className="flex min-w-0 flex-1 items-center gap-2">
                <CardTitle className="min-w-0 flex-1 break-words text-sm font-bold tracking-tight">{t.dashboard.recentClientsTitle}</CardTitle>
