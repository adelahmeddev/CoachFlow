        ...
        {clients.map((client) => {
          ...
          return (
            <StaggerItem key={client.id}>
              <div className="group relative flex flex-col overflow-hidden rounded-2xl border bg-card shadow-soft transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-1 hover:shadow-card-hover hover:border-brand-200 dark:hover:border-brand-900/30">
                {/* top accent */}
                ...
                <div className="relative flex items-start gap-3 p-4 pb-3 min-w-0">
                  <div className="relative shrink-0">
                    <span className={`flex size-12 items-center justify-center rounded-xl text-sm font-extrabold ring-1 ${getAvatarColor(client.id)}`}>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    >
                      <p className="line-clamp-2 text-sm font-bold leading-tight group-hover/link:text-brand-600 dark:group-hover/link:text-brand-400 transition-colors">
                        {client.fullName ?? t.admin.clients.invitedClient}
                      </p>
                    </Link>
                    <div className="mt-0.5 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
                      {client.phone ? (
                        <span dir="ltr" className="min-w-0 flex-1 truncate tabular-nums">{client.phone}</span>
                      ) : (
                  ...
                  {streak > 0 && (
                    <span
                      className={`inline-flex shrink-0 items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-bold text-white shadow-soft transition-transform group-hover:scale-105 ${
                        streak >= 7
                          ? "bg-gradient-to-r from-energy-500 via-amber-500 to-brand-500 animate-streak-radiance"
                          : "bg-gradient-to-r from-energy-500 to-brand-500"
                      }`}
                    >
                      <Flame className={`size-3 fill-white/20 ${streak >= 3 ? "animate-flame" : ""}`} aria-hidden="true" />
                      {streak}
                    </span>
                  )}
                </div>
                ...
                <div className="px-4 pb-3 space-y-2">
                  <div className="flex items-center justify-between rounded-xl border bg-muted/30 px-3 py-2">
                    <span className="inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                      <Trophy className="size-3.5 text-energy-600" aria-hidden="true" />
                      {isAr ? "الباقة" : "Package"}
                    </span>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 text-center">
                    ...
                    <div className="rounded-xl border bg-card p-2">
                      <Dumbbell className="mx-auto size-3.5 text-muscle-500 mb-1" aria-hidden="true" />
                      <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">{isAr ? "التمارين" : "Workouts"}</p>
                      <p className="mt-0.5 text-xs font-medium leading-none tabular-nums">0</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto flex items-center justify-between border-t bg-muted/20 px-4 py-2.5">
                  <Button asChild size="sm" className="h-8 gap-1.5 rounded-[var(--radius-md)] bg-gradient-to-r from-brand-600 to-brand-500 shadow-soft hover:brightness-110">
                  ...
                  <div className="flex items-center gap-1">
                    <Button asChild variant="ghost" size="icon" className="size-8 rounded-[var(--radius-md)] text-muted-foreground hover:text-foreground">
                      <Link href={`/messages?client=${client.id}`} onClick={() => haptics.selection()} aria-label={t.nav.messages}>
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="size-8 rounded-[var(--radius-md)] text-muted-foreground hover:bg-destructive/10 hover:text-destructive"
